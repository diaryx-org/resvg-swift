...
# packages/resvg-swift/uniffi-generated/ — or, with --check, verify that the
# committed binding is byte-for-byte what crates/resvg-swift produces.
#
...
OUT="$ROOT/packages/resvg-swift/uniffi-generated"

echo "▸ Building resvg-swift (host) for bindgen introspection…"
cargo build --manifest-path "$ROOT/Cargo.toml" -p resvg-swift
...
LIB="$ROOT/target/debug/libresvg_swift.dylib"
[ -f "$LIB" ] || LIB="$ROOT/target/debug/libresvg_swift.so"
[ -f "$LIB" ] || { echo "gen-bindings: no libresvg_swift.{dylib,so} under target/debug" >&2; exit 1; }

STAGE="$(mktemp -d)"
trap 'rm -rf "$STAGE"' EXIT

echo "▸ Generating UniFFI Swift binding + C module…"
mkdir -p "$STAGE/out/Sources/ResvgFFI" "$STAGE/out/headers"
...
cargo run -q --manifest-path "$ROOT/Cargo.toml" -p resvg-swift --bin uniffi-bindgen -- \
  generate --library "$LIB" --language swift --no-format --out-dir "$STAGE/gen"
mv "$STAGE/gen/resvg_swift.swift" "$STAGE/out/Sources/ResvgFFI/resvg_swift.swift"
cp "$STAGE/gen/resvg_swiftFFI.h" "$STAGE/out/headers/"
cp "$STAGE/gen/resvg_swiftFFI.modulemap" "$STAGE/out/headers/module.modulemap"

if [ "${1:-}" = "--check" ]; then
  if diff -ru "$OUT" "$STAGE/out"; then
    echo "✓ committed binding matches crates/resvg-swift"
  else
