...
# committed binding is byte-for-byte what crates/resvg-uniffi produces.
#
...
echo "▸ Building resvg-uniffi (host) for bindgen introspection…"
cargo build --manifest-path "$ROOT/Cargo.toml" -p resvg-uniffi
...
LIB="$ROOT/target/debug/libresvg_uniffi.dylib"
[ -f "$LIB" ] || LIB="$ROOT/target/debug/libresvg_uniffi.so"
[ -f "$LIB" ] || { echo "gen-bindings: no libresvg_uniffi.{dylib,so} under target/debug" >&2; exit 1; }
...
if [ "${1:-}" = "--check" ]; then
  if diff -ru "$OUT" "$STAGE/out"; then
    echo "✓ committed binding matches crates/resvg-uniffi"
  else
